...
void firstRun(){  //定义开始第一步骤运动--1------------------------------出发段
   if(digitalRead(red1)==HIGH){//LOW值为0，表示左侧有障碍，车子向右，否则向左 
ll=1;
  }else{       
      ll=2;
    }
...
void secondRun(){  //定义开始第二步骤运动--2----------------------------------转向段
  if(ll==1){//LOW值为0，表示左侧有障碍，车子向右，否则向左 
         right(SPEEDS);//向右转向
         delay(1200);//-----------------------------------------------------------------------------------------------------第一段转向时间控制
         cstop();
         delay(1000);
         rl=1;
  }else{       
       left(SPEEDS);//左转
       delay(1350);//-----------------------------------------------------------------------------------------------------第一段转向时间控制
...
void thiredRun(){//定义开始第三步骤运动--3----------------------------------上桥段
 ...
 analogWrite(PWMA_IN2, SPEEDUP);  //左侧轮子以100转向前运动
  delay(100);
